...
export async function POST(context) {
  ...
  try {
    ...
    return json({ message: '请求格式错误，请刷新页面后重试。' }, 400);
  }
  ...
  try {
    suggestions = await suggestCoverCandidatesFromContent({
      ...
      preferredQueries,
      limit: 3,
      rerankConfig: aiBaseUrl && aiApiKey && aiModel
        ? {
            baseUrl: aiBaseUrl,
            apiKey: aiApiKey,
            model: aiModel,
            objective: [
              `文章标题：${String(payload.title ?? '').trim() || '未提供'}`,
              `文章摘要：${String(payload.description ?? '').trim() || '未提供'}`,
              `文章标签：${String(payload.tags ?? '').trim() || '未提供'}`,
              '请优先挑选最适合作为技术博客封面的图片，要求主题贴合、画面具体、横版友好，不要选图标感太强或信息量太弱的图片。',
            ].join('\n'),
          }
        : undefined,
    });
  ...
  if (!suggestions.length) {
    return json({
      message: preferredQueries.length
        ? '已经结合整篇内容生成了检索词，但当前图片库里仍没有找到足够合适的封面。你可以补充更具体的场景描述后再试。'
        : '暂时没有找到足够合适的封面。当前会优先用关键词检索；如果你已经配置 AI 写作模型，后续会更适合做整篇内容理解匹配。',
    }, 404);
  }
  ...
  return json({
    ...
    message: suggestion.queryMode === 'ai'
      ? `已根据整篇内容生成检索意图，并从多图片源里找到了 ${suggestions.length} 张候选封面。`
      : `已根据标题、标签和正文关键词，从多图片源里找到了 ${suggestions.length} 张候选封面。`,
  });
}
